...
export const BUILTIN_LIST: IndicatorMeta[] = [
  ...
  { id: "hamBbPrice", label: "HAM BB Mum", category: "momentum", pane: "main", acceptsSeries: false, primarySeriesKey: "bbMid", description: "Ana grafik: fiyat Bollinger (close 20/2) + HAM işaretleri (raw H×Y, AL/SAT). Osilatör ölçeğinde HAM çizgisi yok — hamBb alt pane ayrı.", inputs: [num("hamLen", "HAM Hızlı", 21), num("hamLenSlow", "HAM Yavaş", 34), num("rawLen", "Raw Hızlı", 10), num("rawLenSlow", "Raw Yavaş", 21), num("momSpan", "Mom Span", 10), num("normLen", "Norm Len", 80), num("jLen", "Jurik RMA", 20), num("jPhase", "Phase", 0, -100, 100, 1), num("postSmooth", "Final Smooth", 5), num("bbPeriod", "BB Period", 20), num("bbMult", "BB Mult", 2, 0.5, 10, 0.1), num("showMarkers", "HAM işaretleri", 1, 0, 1, 1), num("showCloseTint", "Close rejim rengi", 0, 0, 1, 1)] },
...
export function computeBuiltin(
  ...
  switch (inst.type) {
    ...
    case "hamBb": {
      const showOsc = n(p, "showOsc", 0) !== 0;
      const h = hamBb(candles, {
        hamLen: n(p, "hamLen", 21),
        hamLenSlow: n(p, "hamLenSlow", 34),
        rawLen: n(p, "rawLen", 10),
        rawLenSlow: n(p, "rawLenSlow", 21),
      ...
      const colorRawSlow = c("colorRawSlow", "#ce93d8");
      const colorOsc = c("colorOsc", "#18d0bd66");
      const colorSlow = c("colorSlow", "#ffb74d66");
      const colorBbUp = c("colorBbUp", "#ef535099");
      const colorBbMid = c("colorBbMid", "#8b95a899");
      const colorBbLo = c("colorBbLo", "#26a69a99");
      const plots: PlotSeries[] = [
        line(inst, "zero", "sub", "#8b95a888", candles, candles.map(() => 0), "0"),
        line(inst, "bbUpper", "sub", colorBbUp, candles, h.bbUpper, "BB üst"),
        line(inst, "bbMid", "sub", colorBbMid, candles, h.bbMid, "BB orta"),
        line(inst, "bbLower", "sub", colorBbLo, candles, h.bbLower, "BB alt"),
        line(inst, "oscDisplay", "sub", colorRaw, candles, h.oscDisplay, "Raw hızlı"),
        line(inst, "rawSlow", "sub", colorRawSlow, candles, h.rawSlow, "Raw yavaş"),
      ];
      if (showOsc) {
        plots.push(line(inst, "osc", "sub", colorOsc, candles, h.osc, "HAM Osc"));
        plots.push(line(inst, "oscSlow", "sub", colorSlow, candles, h.oscSlow, "HAM Yavaş"));
      }
      push(plots, {
        oscDisplay: h.oscDisplay,
        rawSlow: h.rawSlow,
        bbUpper: h.bbUpper,
        bbMid: h.bbMid,
        bbLower: h.bbLower,
    ...
    case "hamBbPrice": {
      const showMarkers = n(p, "showMarkers", 1) !== 0;
      const showCloseTint = n(p, "showCloseTint", 0) !== 0;
      ...
      if (showMarkers) {
        // Cap: last 80 bars, key events only, max 40 markers (chart lag)
        const MARK_BARS = 80;
        const MARK_MAX = 40;
        const markers: PlotMarker[] = [];
        const start = Math.max(1, candles.length - MARK_BARS);
        for (let i = start; i < candles.length; i++) {
          ...
          if (h.rawDualDown[i])
            markers.push({ time: tm, position: "aboveBar", color: colorRawSlow, shape: "circle", text: "raw H×Y↓" });
          // lower cross: close crossed above BB lower
          const c0 = h.closes[i - 1];
          const c1 = h.closes[i];
          const l0 = h.bbLower[i - 1];
          const l1 = h.bbLower[i];
          if (
            c0 != null &&
            c1 != null &&
            l0 != null &&
        ...
        if (markers.length > MARK_MAX) {
          midLine.markers = markers.slice(markers.length - MARK_MAX);
        } else if (markers.length) {
          midLine.markers = markers;
        }
      }
      push(plots, {
        bbUpper: h.bbUpper,
