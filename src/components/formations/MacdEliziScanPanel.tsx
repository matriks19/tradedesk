...
import { detectMacdEliziCross, type MacdEliziBias } from "@/lib/scanner/macdEliziScan";
import {
...
const HYBRID_TF_CHIPS: { value: Timeframe; label: string }[] = [
  { value: "15m", label: "15" },
  { value: "30m", label: "30" },
  { value: "1h", label: "60" },
  { value: "2h", label: "120" },
...
const ALL_TFS = HYBRID_TF_CHIPS.map((t) => t.value);
...
export function MacdEliziScanPanel() {
  ...
  const run = useCallback(async (universeOverride?: "market" | "watchlist") => {
    ...
    try {
      ...
      await mapPool(
        ...
        async (job) => {
          try {
            ...
            const hit = detectMacdEliziCross(candles, { maxBarsAgo: lookback });
            if (!hit) return null;
      ...
      const tfLabel = tfs
        .map((t) => HYBRID_TF_CHIPS.find((c) => c.value === t)?.label ?? t)
        .join("/");
      setStatus(
  ...
  const openHit = (r: Row) => {
    ...
    if (pane && !pane.indicators.some((i) => i.type === "macd")) {
      addIndicator(pane.id, "macdEliziHybrid");
    }
  };
  ...
  return (
    <div className="flex flex-col h-full min-h-0 p-2 gap-2">
      ...
      <p className="text-2xs text-desk-muted">
        MACD %60 + Elizi %40 hibrit skor × sinyal kesişimi. Elizi gecikmesini MACD ile dengeler.
        ...
        {sortedRows.map((r) => (
          ...
          >
            <div className="grid grid-cols-[1fr_auto_auto_auto_auto_auto_auto] gap-x-1.5 text-xs items-center font-mono">
              ...
              <span className="text-2xs">{fmtNum(r.hybrid)}</span>
